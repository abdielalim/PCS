package com.abdielalim.appserba.data.local

import android.content.Context
import android.content.SharedPreferences
import android.provider.ContactsContract
import com.abdielalim.appserba.data.model.ActionState
import com.abdielalim.appserba.data.model.AuthUser
import com.abdielalim.appserba.util.getObject
import com.abdielalim.appserba.util.putObject
...
class AuthPref(val context: Context) {
    ...
    suspend fun register(user: AuthUser): ActionState<AuthUser> {
        if (user.email.isBlank() || user.password.isBlank()) {
            ...
            authUser = user
            return ActionState(user, message = "Anda Berhasil Terdaftar")
        }
    }
